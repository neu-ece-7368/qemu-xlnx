...
class ZCmdClient(StoppableThread):
    ...
    _PERIPHERAL_DESCR_REGEX = re.compile(
        r"([0-9]+)\s+([0-9a-zA-Z]+)\s+" r"([0-9a-zA-Z]+)\s*(.*)"
    )
    ...
    def get_gpio_state(self, gpiochip, callback=None):
        """Get GPIO state."""
        self._queue_command(
            self._GPIO_GET_STATE, [gpiochip], callback=callback
        )

    def get_gpio_dir(self, gpiochip, callback=None):
        """Get GPIO state."""
        self._queue_command(self._GPIO_GET_DIR, [gpiochip], callback=callback)
